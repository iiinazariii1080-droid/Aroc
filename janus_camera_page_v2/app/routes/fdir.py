...
@router.get("/ladder", response_model=LadderStatus, summary="Recovery ladder status", dependencies=[ADMIN_DEPENDENCY])
def ladder_status() -> LadderStatus:
    data = get_ladder().status()
    return LadderStatus(**data)


@router.get("/events", summary="Recent FDIR events (newest first)", dependencies=[ADMIN_DEPENDENCY])
def fdir_events(n: int = Query(50, ge=1, le=500)) -> List[Dict[str, Any]]:
    return fdir_recent(n)


@router.get("/mode", response_model=ModeInfo, summary="Current system operating mode", dependencies=[ADMIN_DEPENDENCY])
