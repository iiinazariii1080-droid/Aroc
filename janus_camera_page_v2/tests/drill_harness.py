...
ADMIN_TOKEN = os.getenv("CAM_ADMIN_TOKEN", "")
...
def _get_mode() -> str:
    ...
    try:
        r = httpx.get(f"{BASE_URL}/fdir/mode", timeout=5, headers=_admin_headers())
...
def _get_ladder_level() -> int:
    ...
    try:
        r = httpx.get(f"{BASE_URL}/fdir/ladder", timeout=5, headers=_admin_headers())
