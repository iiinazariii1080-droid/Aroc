...
def ladder(tmp_path):
    ...
    mock_ladder_settings = make_test_settings(
        tmp_path, fdir_dedup_sec=0.0, max_fdir_reboots=2, watchdog_reboot_enabled=True,
    )

    patches = [
        patch("app.services.recovery_ladder._ladder_state_path", new=lambda: ladder_state),
        ...
        patch("app.services.recovery_ladder.get_settings", return_value=mock_ladder_settings),
        # Prevent actual subprocess calls (run_cmd is in app.utils.process)
    ...
    from app.services.recovery_ladder import LadderLevelConfig as LadderLevel, RecoveryLadder
    from app.services.recovery_ladder import RecoveryAction as RA
    ...
    # escalate() is async, so _execute must return an awaitable.
    with patch.object(RecoveryLadder, "_execute", new_callable=AsyncMock, return_value=True):
        obj = RecoveryLadder()

    # Re-attach the mock on the living instance
    obj._execute = AsyncMock(return_value=True)
...
class TestRecoveryLadder:
    ...
    async def test_all_exhausted_enters_safe(self, ladder):
        """After every level is used up, next call → SAFE mode."""
        for _ in range(10):
            await ladder.escalate("sig")
        assert current_mode() == SystemMode.SAFE

    async def test_reset_clears_ladder(self, ladder):
        await ladder.escalate("sig")
        await ladder.escalate("sig")
        ladder.reset()
        s = ladder.status()
...
class TestRebootCircuitBreaker:
    ...
    def test_reset_clears_reboot_counter(self, tmp_path):
        ...
        with (
            ...
            from app.services.recovery_ladder import RecoveryLadder
            _rl = RecoveryLadder()
            _rl._execute = AsyncMock(return_value=True)
            _rl.reset()
...
class TestLadderModeIntegration:
    ...
    async def test_escalation_degrades_mode(self, ladder):
...
class TestNodeLocalReboot:
    ...
    def _make_ladder(self, tmp_path, cam_type, mock_run):
        ...
        from app.services.recovery_ladder import (
            LadderLevelConfig as LadderLevel,
            RecoveryLadder,
        )
        from app.services.recovery_ladder import (
        ...
        p_state = patch("app.services.recovery_ladder._ladder_state_path", new=lambda: tmp_path / "state.json")
        p_rdir = patch("app.services.recovery_ladder._reboot_count_dir", new=lambda: reboot_dir)
        p_rpath = patch("app.services.recovery_ladder._reboot_count_path", new=lambda: reboot_dir / "reboot_count")
        p_marker = patch("app.services.recovery_ladder._reboot_marker_path", new=lambda: reboot_dir / "last_reboot_request")
        p_run = patch("app.utils.process.subprocess.run", mock_run)
        p_dl = patch("app.services.recovery_ladder._default_ladder")
        p_settings = patch("app.services.recovery_ladder.get_settings")

        # Start simple attribute patches first
        simple_patches = [p_state, p_rdir, p_rpath, p_marker, p_run]
        for p in simple_patches:
            p.start()

        # Start callable-mock patches and keep their return mocks
        mock_dl = p_dl.start()
        mock_settings = p_settings.start()
        all_patches = simple_patches + [p_dl, p_settings]

        from tests.conftest import make_test_settings
        settings = make_test_settings(
            tmp_path, camera_type=cam_type, watchdog_reboot_enabled=True,
            max_fdir_reboots=2, fdir_dedup_sec=0.0,
        )
    ...
    async def test_color_node_reboots_locally(self, tmp_path):
        ...
        try:
            result = await rl.escalate("color_cam_fault")
            assert result["action"] == "reboot_node"
...
# ===================================================================
# 6. State Persistence Corruption (T3)
# ===================================================================

@pytest.fixture
def corruption_env(tmp_path):
    """Set up ladder with tmp_path persistence for corruption tests."""
    ladder_state = tmp_path / "fdir_ladder.json"
    reboot_dir = tmp_path / "fdir-persist"
    reboot_dir.mkdir()
    ...
    yield {
        ...
        "reboot_dir": reboot_dir,
        "reboot_count_path": reboot_dir / "reboot_count",
        "settings": settings,
    }
...
class TestStatePersistenceCorruption:
    """Validate that corrupted persistent state is handled gracefully.

    Risk addressed: R02 (Critical) — embedded hardware power loss can
    corrupt fdir_ladder.json or reboot_count. Silent state reset could
    cause infinite reboot loop.
    """

    def test_corrupted_ladder_json_resets_to_level_0(self, corruption_env):
        """Garbage bytes in fdir_ladder.json → loads at level 0, emits WARN."""
        corruption_env["ladder_state"].write_bytes(b"\x80\x81\x82\xff garbage")

        _reset_mode()
        _clear_ring()
        from app.services.recovery_ladder import RecoveryLadder
        rl = RecoveryLadder()
        assert rl.status()["current_level"] == 0

        # Verify corruption detection event was emitted
        events = recent(10)
        corrupt_events = [
            e for e in events
            if "ladder_state_corrupt" in e.get("detection_signal", "")
        ]
        assert len(corrupt_events) >= 1, (
            "Expected FDIR event with 'ladder_state_corrupt' signal on corrupted state"
        )

    def test_truncated_ladder_json_resets_to_level_0(self, corruption_env):
        """Truncated JSON (power loss mid-write) → loads at level 0."""
        corruption_env["ladder_state"].write_text('{"level": 2, "attempts": [1')
    ...
    def test_corrupted_reboot_count_treated_as_zero(self, corruption_env):
        """Non-numeric reboot_count file → returns 0 (not crash)."""
        corruption_env["reboot_count_path"].write_text("not_a_number\n")

        from app.services.recovery_ladder import _read_reboot_count
        count = _read_reboot_count()
        assert count == 0, f"Corrupted reboot_count should return 0, got {count}"

    def test_empty_reboot_count_treated_as_zero(self, corruption_env):
        """Empty reboot_count file (power loss mid-write) → returns 0."""
        corruption_env["reboot_count_path"].write_text("")

        from app.services.recovery_ladder import _read_reboot_count
        count = _read_reboot_count()
        assert count == 0, f"Empty reboot_count should return 0, got {count}"

    def test_reboot_count_exceeds_max_triggers_safe(self, corruption_env):
        """Pre-seeded reboot_count=99 → check_circuit_breaker() → SAFE mode.

        Invariant: reboot counter never exceeds MAX_FDIR_REBOOTS without SAFE.
        """
        corruption_env["reboot_count_path"].write_text("99\n")
        ...
        assert current_mode() == SystemMode.SAFE, (
            "Reboot count 99 exceeds max_fdir_reboots=2 — must enter SAFE mode"
        )

    def test_ladder_state_extra_levels_loads_safely(self, corruption_env):
        """State JSON with more levels than current config → clamped, no crash."""
        import json
        # Write state with level=10 and 10 attempt entries
        # (current config has 4 levels max for color_camera)
        state = {
            "level": 10,
        ...
        corruption_env["ladder_state"].write_text(json.dumps(state))

        _reset_mode()
        from app.services.recovery_ladder import RecoveryLadder
        rl = RecoveryLadder()
        status = rl.status()
        max_levels = len(rl._levels)
        assert status["current_level"] <= max_levels, (
            f"Level {status['current_level']} exceeds max {max_levels} — must be clamped"
        )
