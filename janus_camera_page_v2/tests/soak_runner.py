...
_ADMIN_TOKEN = os.environ.get("CAM_ADMIN_TOKEN", "")


def _admin_headers() -> dict[str, str]:
    """Return admin auth headers if token is configured."""
...
def _poll(base_url: str, timeout: float = 10) -> Sample:
    ...
    try:
        r = httpx.get(f"{base_url}/fdir/mode", timeout=5, headers=admin_h)
        if r.status_code == 200:
    ...
    try:
        r = httpx.get(f"{base_url}/fdir/ladder", timeout=5, headers=admin_h)
        if r.status_code == 200:
    ...
    try:
        r = httpx.get(f"{base_url}/fdir/events?n=5", timeout=5, headers=admin_h)
        if r.status_code == 200:
