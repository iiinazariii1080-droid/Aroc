...
from app.routes.camera import CameraModesResponse, CameraStreamConfig
...
class TestGetCameraModes:
    ...
    async def test_returns_modes(self, mock_modes, client):
        ...
        body = resp.json()
        parsed = CameraModesResponse.model_validate(body)
        assert parsed.pixel_format == "YUYV"
...
class TestGetCameraStreamConfig:
    ...
    async def test_returns_config(self, client, tmp_path):
        ...
        env_mock = MagicMock(env_path=env_file, lock_path=tmp_path / "cam-rgb.lock")
        with patch("app.services.env_store.get_settings", return_value=env_mock):
            resp = await client.get("/config")
        assert resp.status_code == 200
        body = resp.json()
        parsed = CameraStreamConfig.model_validate(body)
    ...
    async def test_defaults_when_env_missing(self, client, tmp_path):
        ...
        env_mock = MagicMock(env_path=missing, lock_path=tmp_path / "cam-rgb.lock")
        with patch("app.services.env_store.get_settings", return_value=env_mock):
            resp = await client.get("/config")
        assert resp.status_code == 200
        body = resp.json()
        parsed = CameraStreamConfig.model_validate(body)
        assert parsed.width == 640  # default
