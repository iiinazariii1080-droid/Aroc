...
async function main() {
  ...
  // ═══════════════════════════════════════════════════════════
  // SECTION F: Error-path tests (T5 — adapter failure modes)
  //
  // Risk addressed: R09 (High) — player hangs silently on
  // adapter error because _onEvent error propagation was
  // untested. These tests use error-injecting Janus stubs.
  // ═══════════════════════════════════════════════════════════

  console.log('\n=== Adapter Error-Path Tests (T5) ===\n');

  // --- ERR Test 1: JanusSessionManager init error callback fires SESSION_ERROR ---
  console.log('--- ERR Test 1: session manager init error callback ---');
  {
    const { sandbox, clock, elements, eventListeners, FakeMediaStream } = createSandbox();

    // Override the Janus stub to call error callback instead of success
    sandbox.Janus = function JanusErrorStub(opts) {
      const inst = {
        opts,
        _connected: false,
        isConnected: () => false,
        attach: () => {},
        destroy: (args) => { if (args && args.success) args.success(); },
      };
      // Call error callback instead of success
      if (opts.error) setTimeout(() => opts.error('Connection refused'), 0);
      return inst;
    };
    sandbox.Janus.randomString = (len) => 'stub' + String(len);
    ...
    const hasError = events.some(e => e.type === 'SESSION_ERROR');
    assert(initFailed || hasError, 'ERR1: init must fail or emit SESSION_ERROR when Janus error callback fires');
  }

  // --- ERR Test 2: streaming adapter attach failure → does not hang ---
  console.log('--- ERR Test 2: streaming adapter attach failure ---');
  {
    const { sandbox, clock } = createSandbox();

    // Janus stub that succeeds on create but fails on attach
    sandbox.Janus = function JanusAttachFailStub(opts) {
      const inst = {
        opts,
        _connected: true,
        isConnected: () => true,
        attach: (pluginOpts) => {
          // Call error callback instead of success
          if (pluginOpts.error) pluginOpts.error('Plugin not found');
        },
        destroy: (args) => { if (args && args.success) args.success(); },
      };
      if (opts.success) opts.success();
    ...
    try {
      await sm.attach('janus.plugin.streaming', {});
    } catch (e) {
      attachFailed = true;
    }
    assert(attachFailed, 'ERR2: attach must reject when Janus calls error callback — not hang');
  }

  // --- ERR Test 3: session manager destroy after error is callable ---
  console.log('--- ERR Test 3: destroy after error ---');
  {
    const { sandbox, clock } = createSandbox();

    sandbox.Janus = function JanusDestroyTestStub(opts) {
      const inst = {
        opts,
        _connected: false,
        isConnected: () => false,
      ...
      // Immediately error
      if (opts.error) opts.error('Connection failed');
      return inst;
    };
    sandbox.Janus.randomString = (len) => 'stub' + String(len);
    ...
    // destroy() after failed init must not throw
    ...
    assert(!destroyFailed, 'ERR3: destroy() after failed init must not throw');

    const events = [];
    sm.onEvent((ev) => events.push(ev));
    await sm.destroy(); // double destroy should also be safe
    assert(events.some(e => e.type === 'SESSION_DESTROYED'), 'ERR3: SESSION_DESTROYED emitted');
  }

  // --- ERR Test 4: DomUIAdapter handles missing DOM elements gracefully ---
  console.log('--- ERR Test 4: DomUIAdapter missing elements ---');
  {
    const clock = createFakeClock();

    // Create sandbox with NO DOM elements
    const emptyDoc = { getElementById: () => null };
    const emptySandbox = vm.createContext({
      window: {},
      console,
      Math: Math,
      Promise: Promise,
      ...
      clearTimeout: clock.clearTimeout,
      document: emptyDoc,
      MediaStream: function() { return { getTracks: () => [], addTrack: () => {}, removeTrack: () => {} }; },
      Date: Date,
    });
    emptySandbox.Math.random = () => 0.5;

    loadScript(emptySandbox, path.join(root, 'player', 'ns.js'));
    loadScript(emptySandbox, path.join(root, 'player', 'core', 'player_state.js'));
    loadScript(emptySandbox, path.join(root, 'player', 'core', 'codes.js'));
    loadScript(emptySandbox, path.join(root, 'player', 'adapters', 'logger.js'));
    loadScript(emptySandbox, path.join(root, 'player', 'adapters', 'dom_ui_adapter.js'));

    const AP = emptySandbox.window.AutonomousPlayer;
    const log = { debug: () => {}, info: () => {}, warn: () => {}, error: () => {} };

    let constructFailed = false;
    try {
      const ui = new AP.Adapters.DomUIAdapter({
        videoElId: 'nonexistent_video',
        playBtnId: 'nonexistent_btn',
        statsBtnId: 'nonexistent_stats',
        statsBoxId: 'nonexistent_box',
        statusPillId: 'nonexistent_pill',
      }, log);
      // If construction succeeds, verify it degrades gracefully
      assert(true, 'ERR4: DomUIAdapter constructed without throwing on missing elements');
    } catch (e) {
      // If it throws, that's also acceptable — it should NOT hang silently
      constructFailed = true;
      assert(true, 'ERR4: DomUIAdapter threw on missing elements (acceptable)');
    }
  }

  // --- ERR Test 5: _emit swallows throwing observer without killing session ---
  console.log('--- ERR Test 5: _emit swallows throwing observer ---');
  {
    const { sandbox } = createSandbox();
    loadScript(sandbox, path.join(root, 'player', 'ns.js'));
    loadScript(sandbox, path.join(root, 'player', 'core', 'player_state.js'));
    ...
    // First observer throws, second should still receive events
    sm.onEvent(() => { throw new Error('observer crash'); });
    sm.onEvent((ev) => receivedEvents.push(ev));

    // Must not throw
    sm._emit('ERROR', { where: 'test', error: 'simulated' });

    assert(receivedEvents.length === 1, 'ERR5: second observer received event despite first throwing');
    assert(receivedEvents[0].type === 'ERROR', 'ERR5: event type is ERROR');
    assert(receivedEvents[0].payload.where === 'test', 'ERR5: error payload preserved');
  }

  // --- ERR Test 6: recreate after error recovers cleanly ---
  console.log('--- ERR Test 6: recreate after error ---');
  {
    const { sandbox, clock, JanusStub } = createSandbox();

    let callCount = 0;
    // First call errors, second call succeeds
    sandbox.Janus = function JanusRecoverStub(opts) {
      callCount++;
      const inst = {
        opts,
        _connected: callCount > 1,
        isConnected: () => inst._connected,
        attach: (pluginOpts) => {
          if (pluginOpts.success) pluginOpts.success({ plugin: pluginOpts.plugin });
        },
        destroy: (args) => {
          inst._connected = false;
          if (args && args.success) args.success();
      ...
      if (callCount === 1) {
        // First attempt: error
        if (opts.error) opts.error('Network unreachable');
      } else {
    ...
    // Recreate should succeed (second Janus constructor call succeeds)
    await sm.recreate({ iceServers: [], iceTransportPolicy: 'all' });

    assert(firstFailed, 'ERR6: first init must fail');
    assert(sm.isAlive(), 'ERR6: session must be alive after successful recreate');
    assert(callCount >= 2, 'ERR6: Janus constructor called at least twice');
    const hasRecreated = events.some(e => e.type === 'SESSION_RECREATED');
    assert(hasRecreated, 'ERR6: SESSION_RECREATED emitted after recovery');
  }
