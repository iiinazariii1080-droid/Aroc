(function(){
  ...
  const defaultSeverityForReason = AP.App.RecoveryMap.defaultSeverityForReason;
  ...
  class PlayerController {
    ...
    _applySnapshot(next, errorCode){
      ...
      if (next.state === PlayerState.ERROR) {
        ...
        // NOTE: _scheduleErrorAutoRetry() is called AFTER _executeActions() in
        // handleEvent() so that CANCEL_ALL_TIMERS does not immediately clear it.
      }
    ...
    handleEvent(event){
      ...
      // Schedule error auto-retry AFTER actions are executed so that
      // CANCEL_ALL_TIMERS (part of fail-closed transitions) does not
      // immediately clear the timer that was just scheduled.
      if (result.next.state === PlayerState.ERROR) {
