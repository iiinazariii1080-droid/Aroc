...
async def joystick_frame(request: Request, frame: JoystickFrame):
    now = time.time()
    _LOG.info(f"joystick_frame received at {now:.6f}, client_ts={frame.ts:.6f}, delay={now - frame.ts:.3f}s")
    ...
    try:
        while True:
            try:
                ...
                _LOG.info("joystick_websocket: client disconnected")
                break
            except Exception as exc:
                await websocket.send_json(
...
async def get_symovo_teleop_config(request: Request) -> SymovoTeleopConfigResponse:
    ...
    if jp is None:
        raise HTTPException(status_code=503, detail="joystick_pipeline_unavailable")
    cfg = jp.get_symovo_teleop_config()
    return SymovoTeleopConfigResponse(
        duration=min(max(cfg["duration"], SYMOVO_TELEOP_DURATION_MIN), SYMOVO_TELEOP_DURATION_MAX),
        linear_m_s=min(max(cfg["linear_m_s"], SYMOVO_TELEOP_LINEAR_MIN), SYMOVO_TELEOP_LINEAR_MAX),
        angular_rad_s=min(max(cfg["angular_rad_s"], SYMOVO_TELEOP_ANGULAR_MIN), SYMOVO_TELEOP_ANGULAR_MAX),
    )
...
async def put_symovo_teleop_config(
    ...
    if jp is None:
        raise HTTPException(status_code=503, detail="joystick_pipeline_unavailable")
    jp.update_symovo_teleop_config(
        duration=body.duration,
        linear_m_s=body.linear_m_s,
        angular_rad_s=body.angular_rad_s,
    )
    cfg = jp.get_symovo_teleop_config()
    return SymovoTeleopConfigResponse(**cfg)
