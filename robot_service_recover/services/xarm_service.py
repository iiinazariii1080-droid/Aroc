...
class XarmManipulatorClient:
    ...
    async def complex_move_with_joints(self, params: MoveWithJointsDictParams):
        return await self._post("/complex_move/with_joints_dict", json=params.model_dump(), timeout=self._infinite_timeout)

    @guarded_async_call(xarm_lock)
    async def move_with_joints(self, params: MoveWithJointsParams):
        return await self._post("/move/change_joints", json=params.model_dump(), timeout=self._infinite_timeout)

    @guarded_async_call(xarm_lock)
    async def move_to_pose(self, params: MoveWithPoseParams):
        return await self._post("/move/change_pose", json=params.model_dump(), timeout=self._infinite_timeout)

    @guarded_async_call(xarm_lock)
    async def change_tool_position(self, params: MoveWithToolParams):
        return await self._post("/move/change_tool_position", json=params.model_dump(), timeout=self._infinite_timeout)

    @safe_call
    async def gripper_drop(self) -> Dict[str, Any]:
        return await self._post("/gripper/drop")

    @safe_call
    async def gripper_take(self) -> Dict[str, Any]:
        return await self._post("/gripper/take")
