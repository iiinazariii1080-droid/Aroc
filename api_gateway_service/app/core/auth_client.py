...
class AuthClient:
    def __init__(
        ...
        *,
        ...
        self._auth_connect_timeout_s = auth_connect_timeout_s
    ...
    async def _seconds_until_refresh(self) -> float:
        async with self._lock:
            if not self._token:
                ...
                backoff = min(
                    self._min_retry_interval_s * (2 ** self._consecutive_failures),
    ...
    async def _refresh(self, *, reason: str, raise_on_error: bool) -> None:
        ...
        try:
            ...
            data = await request_robot_token(client, context)
            expires_in = data.get("expires_in", 0)
            async with self._lock:
                self._token = data["access_token"]
                self._expires_at = time.time() + max(float(expires_in), 0.0)
            ...
            if isinstance(exc, httpx.HTTPStatusError):
                ...
                metric_reason = f"http_status_{exc.response.status_code}"
            elif isinstance(exc, httpx.ConnectTimeout):
                metric_reason = "connect_timeout"
            elif isinstance(exc, httpx.ReadTimeout):
                metric_reason = "read_timeout"
            elif isinstance(exc, httpx.ConnectError):
                metric_reason = "connect_error"
            elif isinstance(exc, httpx.HTTPError):
                metric_reason = "http_error"
            elif isinstance(exc, RuntimeError):
                metric_reason = "runtime_error"
            ...
            # Use DEBUG after repeated failures to avoid log spam
            if failures <= 3:
                logger.warning("Token refresh failed (reason=%s): %s", reason, error_text, extra={"reason": reason})
            else:
                logger.debug(
