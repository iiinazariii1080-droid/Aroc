...
from tests.test_utils.assertions import Eventually, Always
from tests.test_utils.config import TestConfig
from tests.test_utils.logging import TestLogger
...
@pytest.mark.asyncio
async def test_is_motion(drive, test_config: TestConfig) -> None:
    """Test /is_motion API (is_motion).
    ...
    """
    logger = TestLogger("test_is_motion")
    logger.log_stage("start")
    ...
    # Test 1: Check is_motion when stationary
    is_moving = await drive.is_motion()
    logger.log_stage("motion_check_stationary", is_motion=is_moving)
    debug_log("test_api_functions.py:is_motion_stationary", "Checking motion when stationary", {
        "is_motion": is_moving,
    })
    
    assert isinstance(is_moving, bool), f"is_motion should return bool, got {type(is_moving)}"
    
    # When stationary, should be False (target_reached should be True)
    status = await drive.get_status()
    if status.get("target_reached", True):
        assert not is_moving, "is_motion should be False when target_reached is True"
    
    # Test 2: Check is_motion during movement (if we can start a move)
    ...
    try:
        ...
        is_moving_during_move = await drive.is_motion()
        logger.log_stage("motion_check_moving", is_motion=is_moving_during_move)
        debug_log("test_api_functions.py:is_motion_moving", "Checking motion during move", {
            "is_motion": is_moving_during_move,
        })
        
        # Should be True if movement started
        status_during = await drive.get_status()
        if not status_during.get("target_reached", True):
            assert is_moving_during_move, "is_motion should be True when target_reached is False"
        ...
        debug_log("test_api_functions.py:is_motion_move_failed", "Move failed (may need homing)", {
            "error": str(e),
        })
...
async def test_move_to_position_with_homing(drive, test_config: TestConfig) -> None:
    ...
    # Step 2.1: Verify homing mode was set correctly (M2 compliance check)
    ...
    # Step 5: Verify Profile Position mode was set correctly (M2 compliance check)
