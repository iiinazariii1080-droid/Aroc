...
3. Homing выполнен (если обязателен по сценарию).
   > **Примечание:** OD 0x6098 (Homing Method) на dryve D1 доступен только для чтения.
   > Сервис не пишет его по умолчанию (`skip_method_write=True`), используется
   > метод, сохранённый в приводе (настраивается через dryve ProDrive).
