...
class ManipulatorCommands:
    ...
    async def move_step(self, direction: str, acc: int = 500, mode: int = 0, is_move_tool: bool = True, is_loop: bool = True, wait_response: bool = False, timeout: float = 5.0, ttl_ms: Optional[int] = None):
        """Single step movement. When is_loop=True, TTL (ttl_ms) defines max validity of the command in seconds for downstream/safety."""
        data = {
            "direction": direction,
            "isLoop": is_loop,
        ...
        if ttl_ms is not None and ttl_ms > 0:
            data["ttl"] = ttl_ms / 1000.0  # seconds for safety/controller
        payload = self._build("xarm_move_step", data)
