...
class IgusMotorClient:
    def __init__(self, base_url: Optional[str] = None, timeout_seconds: int = 10, operation_timeout_seconds: Optional[float] = None, motion_timeout_seconds: Optional[float] = None):
        if base_url is None:
            try:
                ...
                base_url = "http://192.168.1.10:8101"
    ...
    @guarded_async_call(motor_lock)
    async def jog_start(self, *, direction: str, speed: Optional[float] = None, ttl_ms: int = 200, duration_ms: Optional[int] = None) -> Dict[str, Any]:
        ...
        data = await self._post("/drive/jog_start", json=payload)
        return self._unwrap_envelope(data)

    @guarded_async_call(motor_lock)
    async def jog_update(self, *, direction: str, speed: Optional[float] = None, ttl_ms: int = 200, duration_ms: Optional[int] = None) -> Dict[str, Any]:
        payload: Dict[str, Any] = {
            "direction": direction,
            "ttl_ms": int(ttl_ms),
        }
        ...
        if duration_ms is not None:
            payload["duration_ms"] = int(duration_ms)
        data = await self._post("/drive/jog_update", json=payload)
        return self._unwrap_envelope(data)

    @guarded_async_call(motor_lock)
    async def jog_stop(self) -> Dict[str, Any]:
        data = await self._post("/drive/jog_stop", json={})
        return self._unwrap_envelope(data)
