...
class XarmStatusResponse(BaseModel):
    ...
    safety_lockout_hint: bool = False
...
class WebSocketInfo(BaseModel):
    endpoint: str = Field(description="Service endpoint that accepts WebSocket connections")
    backend_url: str = Field(description="Upstream xArm backend WebSocket URL the proxy tunnels to")
    description: str = Field(description="Short description of what the tunnel does")
    query_param_forwarding: str = Field(description="Explains how query parameters are forwarded")
    example_command: Optional[str] = Field(
        default=None,
        description="Example client command showing how to connect",
    )
